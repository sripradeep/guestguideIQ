...
| Header | Value | Why |
|---|---|---|
| `Content-Security-Policy` | `default-src 'self'`; `script-src 'self' '<sha256 hashes>'`; `connect-src 'self' <api-origin>`; `img-src 'self' data:`; `object-src 'none'`; `base-uri 'self'`; `frame-ancestors 'none'` | The primary XSS mitigation. `connect-src` must name the API origin explicitly **because the frontend is cross-origin** — under the proxy it would be `'self'` alone |
...
**Hashes, not nonces — and this is a real consequence of the hosting choice.** A
nonce must be unique per response, which needs a server that renders the HTML. There
isn't one. **Use hash-based `script-src`**, generated at build time from the emitted
inline scripts and injected into the header policy by the deploy step.

That is workable, but it is a **build/deploy coupling that must not be manual**: if
someone edits the header policy by hand, or adds an inline script without
regenerating the hashes, the app breaks in the browser and nowhere in CI. **The
policy must be generated by the build and applied by the deploy**, and the
post-deploy check must load a real page and confirm no CSP violation fires.

**`'unsafe-inline'` is not an acceptable escape hatch here**, given that a successful
injection yields a week-long, non-revocable compromise. If hash generation proves
awkward, the correct answer is to emit no inline scripts at all — which Vite can be
configured to do — not to relax the policy.
