...
> **Round 2 (confirmed)**: adds per-locality contrast validation for the new BrandTheme component (FR9) — a locality-brand's chosen accent color is a new source of AA risk that a single global token check does not cover.
...
- [ ] Every locality-brand's chosen accent color (BrandTheme) is independently contrast-checked at creation/edit time on AD-3 — a single global-token check is not sufficient once accent color is locality-configurable; a locality-brand that fails the floor must not be savable with visual styling applied (falls back to `minimal-brand` instead).
...
- The exact mechanism for enforcing per-locality accent-color contrast at save time on AD-3 (client-side pre-check, server-side validation, or both) — the requirement is established here (see Perceivable checklist above); the implementation mechanics are deferred to `domain-design`/`functional-design`.
